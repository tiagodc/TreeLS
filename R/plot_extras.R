...
tlsPlot.dh = function(las, pars, clear=T, wired=T, col='white'){
  if('ax' %in% names(pars)){
    tlsPlot.dh.bf.cylinder(las, pars$x, pars$y, pars$d/200, pars$ax, pars$ay, clear, wired, col)
  }else if('rho' %in% names(pars)){
    tlsPlot.dh.cylinder(las, pars$rho, pars$theta, pars$phi, pars$alpha, pars$d/200, clear, wired, col)
  }else{
    tlsPlot.dh.circle(las, pars$X, pars$Y, pars$d/200, clear, wired, col)
  }
}
