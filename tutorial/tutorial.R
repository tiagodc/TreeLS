...
bg3d('black')
rgl.points(M1.stem[[1]], col=cloud.col(M1.stem[[1]]))
rgl.points(M1.trunk, col='darkred')
rgl.points(tree, col='grey', size=1)

M1.conic = taper.mod(stem = M1.stem[[2]], tree = tree, rad.max = .3)
M1.eval = real.vs.est(tree = tree, stem = M1.stem$stem.points, 
...
bg3d('black')
rgl.points(M2.stem[[1]], col=cloud.col(M2.stem[[1]]))
rgl.points(M2.trunk, col='darkred')
rgl.points(tree, col='grey', size=1)

M2.conic = taper.mod(stem = M2.stem[[2]], tree = tree, l.int = .5, method = 'cylinder', rad.max = .3)
M2.eval = real.vs.est(tree = tree, stem = M2.stem[[1]],
                      d = proof$d[proof$tree == trn], H = proof$h[proof$tree == trn],
...
bg3d('black')
rgl.points(M3.stem[[1]], col=cloud.col(M3.stem[[1]]))
rgl.points(M3.trunk, col='darkred')
rgl.points(tree, col='grey', size=1)

M3.conic = taper.mod(stem = M3.stem[[2]], tree = tree, l.int = .5, method = 'cylinder', rad.max = .3)
M3.eval = real.vs.est(tree = tree, stem = M3.stem[[1]],
                      d = proof$d[proof$tree == trn], H = proof$h[proof$tree == trn],
